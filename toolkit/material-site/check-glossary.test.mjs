...
function fixture(term) {
  ...
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    '<p><a href="../fogalomtar/#scope">scope</a> segít kijelölni a munka határait.</p>',
  );
  return root;
}
const valid = {
  slug: "scope",
  preferred: "a munka határai",
  english: "scope",
  definitionHu: "A vállalt munka pontos határa.",
  aliases: ["hatókör"],
  avoid: [],
  related: [],
  usedIn: ["/materials/modulok/"],
};
...
test("valid registry and exact first-use link pass", () => {
  ...
  assert.deepEqual(
    validateGlossary({
      source: path.join(root, "glossary.json"),
      site: root,
      phase: "final",
    }),
    [],
...
test("plain h1 labels do not steal glossary first use from the explanatory lead", () => {
  const root = fixture(valid);
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    `
    <main><h1>Scope</h1>
    <p>A <a href="../fogalomtar/#scope">scope</a> kijelöli a feladat határát.</p></main>`,
  );
  assert.deepEqual(
    validateGlossary({
      source: path.join(root, "glossary.json"),
...
test("a Hungarian inflected term remains a valid first use", () => {
  const root = fixture({
    ...valid,
    preferred: "elfogadási feltétel",
    english: "acceptance criterion",
    aliases: [],
  });
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    '<main><p>Az <a href="../fogalomtar/#scope">elfogadási feltételekhez</a> ellenőrzés tartozik.</p></main>',
  );
  assert.deepEqual(
    validateGlossary({
      source: path.join(root, "glossary.json"),
...
test("assimilated and possessive Hungarian forms remain valid first uses", () => {
  const root = fixture({
    ...valid,
    preferred: "munkadarab",
    english: "artifact",
    aliases: [],
  });
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    '<main><p>A <a href="../fogalomtar/#scope">munkadarabbal</a> igazoljuk az eredményt.</p></main>',
  );
  assert.deepEqual(
    validateGlossary({
      source: path.join(root, "glossary.json"),
  ...
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    '<main><p>A <a href="../fogalomtar/#scope">szabályainkhoz</a> ellenőrzés tartozik.</p></main>',
  );
  fs.writeFileSync(
    path.join(root, "glossary.json"),
    JSON.stringify({
      schemaVersion: 2,
      registryVersion: "2.0.0",
      terms: [{ ...valid, preferred: "szabály", english: "rule", aliases: [] }],
    }),
  );
  assert.deepEqual(
    validateGlossary({
...
test("Hungarian multigraph stems accept their correctly assimilated surface forms", () => {
  ...
  for (const [preferred, surface] of examples) {
    ...
    fs.writeFileSync(
      path.join(root, "materials/modulok/index.html"),
      `<main><p>A <a href="../fogalomtar/#scope">${surface}</a> ellenőrizzük a hasonulást.</p></main>`,
    );
    assert.deepEqual(
      validateGlossary({
        source: path.join(root, "glossary.json"),
...
test("multigraph assimilation does not accept unrelated derivations", () => {
  ...
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    '<main><p>A szabályozás más jelentésű szó.</p><p>A <a href="../fogalomtar/#scope">szabály</a> később linkelt.</p></main>',
  );
  assert.deepEqual(
    validateGlossary({
      source: path.join(root, "glossary.json"),
...
test("glossary page accepts its exact local self anchor", () => {
  ...
  fs.writeFileSync(
    path.join(root, "materials/fogalomtar/index.html"),
    '<main><p><a href="#scope">scope</a> segít kijelölni a munka határait.</p></main>',
  );
  fs.writeFileSync(
    path.join(root, "glossary.json"),
    JSON.stringify({
      schemaVersion: 2,
      registryVersion: "2.0.0",
      terms: [{ ...valid, usedIn: ["/materials/fogalomtar/"] }],
    }),
  );
  assert.deepEqual(
    validateGlossary({
...
test("foundation validates registry relations but defers future-content first use", () => {
  ...
  fs.writeFileSync(
    path.join(root, "materials/modulok/index.html"),
    "<p>Semleges foundation fixture.</p>",
  );
  assert.deepEqual(
    validateGlossary({
      source: path.join(root, "glossary.json"),
  ...
  assert.ok(
    validateGlossary({
      ...
      site: root,
      phase: "final",
    }).some((x) => x.includes("does not use the term")),
  );
});
