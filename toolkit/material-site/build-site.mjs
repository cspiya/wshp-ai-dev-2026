...
function usage(msg) {
  ...
  console.error(
    "usage: node toolkit/material-site/build-site.mjs [--clean] [--out <dir>] [--phase foundation|incremental|final]",
  );
  process.exit(2);
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--clean") opts.clean = true;
      ...
      opts.out = argv[++i] ?? usage("--out needs a value");
    else if (a === "--phase")
      opts.phase = argv[++i] ?? usage("--phase needs a value");
    else usage(`unknown option: ${a}`);
  }
  if (!["foundation", "incremental", "final"].includes(opts.phase))
    usage(`invalid phase: ${opts.phase}`);
  return opts;
}
...
export function validateManifest(manifest) {
  ...
  for (const r of manifest.routes) {
    for (const field of ["id", "source", "output", "title", "owner"]) {
      if (typeof r[field] !== "string" || r[field].length === 0)
        errors.push(`${r.id ?? "?"}: missing ${field}`);
    }
    ...
    if (!r.id.startsWith("/") || !r.id.endsWith("/"))
      errors.push(`route id must start and end with /: ${r.id}`);
    const key = `${r.parent}::${r.order}`;
    if (orderKeys.has(key)) errors.push(`duplicate (parent, order): ${key}`);
    orderKeys.add(key);
  }
  for (const r of manifest.routes) {
    if (r.parent !== null && !ids.has(r.parent))
      errors.push(`${r.id}: parent ${r.parent} not in manifest`);
    if (
      typeof r.overviewQuestion !== "string" ||
      r.overviewQuestion.length === 0
    ) {
      errors.push(
        `${r.id}: missing overviewQuestion (v3.4 — every teaching page needs an overview)`,
      );
      continue;
    }
    ...
    if (!Array.isArray(r.overviewCovers) || r.overviewCovers.length !== 4)
      errors.push(
        `${r.id}: overviewCovers must contain the four big-picture categories`,
      );
    ...
    for (const v of r.visualQuestions) {
      ...
      if (typeof v.question !== "string" || v.question.length === 0)
        errors.push(`${r.id}: visual question without text`);
      if (!VISUAL_QUESTION_TYPES.includes(v.type))
        errors.push(`${label}: invalid type "${v.type}"`);
      const coversAllGlossaryTerms = v.glossaryCoverage === "all";
      if (v.glossaryCoverage != null && !coversAllGlossaryTerms)
        errors.push(`${label}: glossaryCoverage must be all when declared`);
      if (coversAllGlossaryTerms && v.glossarySlugs != null)
        errors.push(
          `${label}: glossaryCoverage all cannot freeze glossarySlugs`,
        );
      if (
        !coversAllGlossaryTerms &&
        (!Array.isArray(v.glossarySlugs) || v.glossarySlugs.length === 0)
      )
        errors.push(
          `${label}: glossarySlugs must contain canonical glossary slugs`,
        );
      if (!["page-local", "shared"].includes(v.coverage))
        errors.push(
          `${label}: undisposed — coverage must be page-local or shared; complex questions cannot stay prose-only`,
        );
      if (!/^fig-[a-z0-9-]+$/.test(v.diagramId ?? ""))
        errors.push(`${label}: missing or invalid diagramId`);
    }
  }
  ...
  if (glossaryOwners.length === 0)
    errors.push(
      "glossary ownership missing — no route with owner GLOSSARY to carry the shared search terms",
    );
  if (glossaryOwners.length > 1)
    errors.push(
      `duplicate glossary ownership — ${glossaryOwners.length} routes claim owner GLOSSARY`,
    );
...
function validateCompatibilityRoutes(
  ...
  for (const [index, compat] of block.routes.entries()) {
    ...
    const normalizedOutput =
      typeof compat.output === "string" ? compat.output.toLowerCase() : "";
    if (legacyOutputs.has(normalizedOutput))
      errors.push(`${label}: duplicate compatibility output ${compat.output}`);
    legacyOutputs.add(normalizedOutput);
    if (canonicalOutputs.has(compat.output))
      errors.push(
        `${label}: compatibility output collides with a canonical output`,
      );
    if (
      typeof compat.canonical !== "string" ||
      !canonicalIds.has(compat.canonical)
    ) {
      errors.push(
    ...
    const alias =
      ...
      compat.output.startsWith("materials/")
        ...
        : null;
    const canonicalRoute = manifest.routes.find(
      (route) => route.alias === alias,
    );
    if (canonicalRoute && compat.canonical !== canonicalRoute.id) {
      ...
      errors.push(
        `${label}: no canonical route owns legacy alias ${alias ?? "?"}`,
      );
    }
  }
}
...
function loadManifest() {
  const manifest = JSON.parse(
    fs.readFileSync(path.join(HERE, "site-manifest.json"), "utf8"),
  );
...
function loadGlossary(phase, substitutions, errors) {
  ...
  if (!fs.existsSync(canonical)) {
    if (phase === "final") {
      errors.push(
        "materials/fogalomtar/glossary.json missing — final phase requires the accepted registry",
      );
      return null;
    }
    ...
    if (!fs.existsSync(source)) {
      errors.push(
        "foundation glossary fixture missing (fixtures/site/glossary.json)",
      );
      return null;
    }
  ...
  try {
    ...
    errors.push(
      `${path.relative(REPO_ROOT, source)}: glossary registry is not valid JSON (${err.message})`,
    );
    return null;
  }
  ...
  if (registry.schemaVersion !== 2) {
    errors.push(
      `${label}: glossary schemaVersion must be 2 (got ${registry.schemaVersion})`,
    );
    return null;
  }
  ...
  for (const t of registry.terms) {
    if (
      ...
      t.slug.length === 0 ||
      typeof t.preferred !== "string" ||
      t.preferred.length === 0
    ) {
      errors.push(
        `${label}: glossary term missing slug/preferred (${JSON.stringify(t).slice(0, 60)})`,
      );
      return null;
    }
  }
...
export function routeDisposition(phase, routeId, acceptedSourceExists) {
  if (phase === "foundation") {
    return FOUNDATION_CANONICAL_ROUTES.has(routeId)
      ? acceptedSourceExists
        ? "real"
        : "missing"
      : "fixture";
  }
...
function buildNavModel(manifest) {
  ...
  const topLevel = manifest.routes
    .filter((r) => r.parent === "/")
...
function renderSidebarTree(route, nav) {
  ...
  const trailIds = new Set(
    [...ancestors(route, nav.byId), route].map((r) => r.id),
  );
  ...
  const subtree = (r) => {
    ...
    const items = kids
      .map((k) => {
        const kidTree =
          (nav.children.get(k.id) ?? []).length > 0 ? subtree(k) : null;
...
function renderToc(headings) {
  ...
  const items = h2s
    .map((h) => `<li><a href="#${h.a}">${escapeHtml(h.t)}</a></li>`)
    .join("");
  ...
  const progress =
    moduleIndex >= 0
      ? `<p class="module-progress" aria-label="Haladás a modulokban"><span class="progress-track" aria-hidden="true">${nav.modules
          .map(
            (m, i) =>
              `<span class="progress-dot${i <= moduleIndex ? " done" : ""}"></span>`,
          )
...
function injectHead(html, route) {
  let out = html;
  out = out.replace(
    /<meta[^>]+http-equiv\s*=\s*["']Content-Security-Policy["'][^>]*>\s*/gi,
    "",
  );
  out = out.replace(
...
function selfTest() {
  ...
  for (const [file, expected] of cases) {
    const fixture = JSON.parse(
      fs.readFileSync(path.join(FIXTURES, "manifests", file), "utf8"),
    );
    const errors = validateManifest(fixture);
    if (!errors.some((e) => e.includes(expected))) {
      console.error(
        `[build-site] self-test FAILED: ${file} did not produce "${expected}" (got: ${errors.join(" | ") || "no errors"})`,
      );
      ok = false;
    }
  }
  ...
  if (validateManifest(real).length > 0) {
    console.error(
      "[build-site] self-test FAILED: the shipped manifest does not validate cleanly",
    );
    ok = false;
  }
  if (!ok) process.exit(1);
  console.log(
    `[build-site] self-test passed (${cases.length} invalid manifest fixtures rejected for the intended reason, shipped manifest clean)`,
  );
  process.exit(0);
}
...
function main() {
  ...
  if (opts.clean && fs.existsSync(outRoot))
  ...
  for (const route of manifest.routes) {
    ...
    const acceptedSourceExists =
      fs.existsSync(srcPath) && fs.existsSync(diagramRegistryFor(route));
    ...
    if (disposition === "fixture") {
      ...
      if (!fs.existsSync(fixture)) {
        errors.push(
          `${route.id}: preview fixture ${path.basename(fixture)} not found`,
        );
        continue;
      }
      html = fs
      ...
      errors.push(
        `${route.id}: accepted canonical page unit missing (${route.source} + media/diagrams.json) — ${opts.phase} phase requires this route`,
      );
      continue;
    }
    ...
    if (!/(<body)(\s[^>]*)?>/i.test(html) || !/<\/body>/i.test(html)) {
      errors.push(
        `${route.id}: source has no <body>…</body> pair — shell cannot be injected`,
      );
      continue;
    }
    ...
    html = html.replace(
      ...
      (m, routeId, fragment = "") => {
        ...
        if (!target) {
          errors.push(
            `${route.id}: data-route target not in manifest: ${routeId}`,
          );
          return m;
        }
    ...
    if (!usedFixture) {
      copyDirIfExists(
        path.join(path.dirname(srcPath), "media"),
        path.join(path.dirname(destPath), "media"),
      );
      ...
      for (const match of html.matchAll(
        ...
        if (
          !localSource.startsWith(`${sourceDir}${path.sep}`) ||
          !fs.existsSync(localSource)
        )
          continue;
  ...
  for (const compat of manifest.compatibilityRoutes.routes) {
    ...
    if (!target) {
      errors.push(
        `${compat.output}: compatibility target missing: ${compat.canonical}`,
      );
      continue;
    }
    ...
    fs.writeFileSync(
      ...
      renderCompatibilityPage(
        compat,
        target,
        manifest.compatibilityRoutes.sunset,
      ),
    );
  ...
  const indexBody = JSON.stringify(
    { pages: searchEntries, glossary: glossaryBlock },
    null,
    1,
  );
  fs.writeFileSync(
