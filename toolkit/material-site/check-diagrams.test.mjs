...
function fixture({ diagrams, questions, owner = 'SHELL-BUILD' } = {}) {
  ...
  const route = { id: '/page/', source: 'page/index.html', output: '.site/page/index.html', owner, overviewDiagramId: 'flow', visualQuestions };
  fs.writeFileSync(path.join(root, 'toolkit/material-site/site-manifest.json'), JSON.stringify({ routes: [route] }));
  fs.writeFileSync(path.join(root, 'page/index.html'), '<h1>Forrás</h1>');
  const figures = records.filter((item) => (item.record ?? item).disposition === 'page-local').map((item) => { const d = item.record ?? item; return `<figure id="${d.id}"><img src="media/${d.output}" alt="A folyamat" width="400" height="200"><figcaption>Folyamat</figcaption><p id="${d.id}-text">Teljes szöveges magyarázat.</p></figure>`; }).join('');
