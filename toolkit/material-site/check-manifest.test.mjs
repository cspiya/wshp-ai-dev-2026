...
function compatibilityRoutes(overrides = {}) {
  return {
    ...
    routes: [...FROZEN_COMPATIBILITY_OUTPUTS].map((output) => {
      ...
      const canonical = [...FROZEN_PRESENTATION].find(
        ([, presentation]) => presentation[1] === alias,
      )?.[0];
...
function fixture(routes, compatibility = compatibilityRoutes()) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ routes, compatibilityRoutes: compatibility }),
  );
  return root;
}
function route(id = "/", overrides = {}) {
  ...
  const suffix =
    id === "/" ? "root" : id.replaceAll("/", "-").replace(/^-|-$/g, "");
  return {
    id,
    ...frozen,
    title: presentation[0],
    alias: presentation[1],
    overviewQuestion: "Hogyan áll össze a workshop?",
    overviewDiagramId: `fig-overview-${suffix}`,
    overviewCovers: [
      "page-purpose",
      "learner-value",
      "main-relationships",
    ...
    visualQuestions: [
      {
        id: `q-overview-${suffix}`,
        question: "Hogyan áll össze?",
        type: "relationship",
        glossarySlugs: ["agent-ready-repo"],
        coverage: "page-local",
        diagramId: `fig-overview-${suffix}`,
        takeaway: "A módszer részei egymásra épülnek.",
      },
    ],
    ...overrides,
  };
...
function canonicalRoutes() {
  return [...FROZEN_ROUTES.keys()].map((id) => route(id));
}
function writeGlossary(root, slugs) {
  fs.mkdirSync(path.join(root, "materials/fogalomtar"), { recursive: true });
  fs.writeFileSync(
    path.join(root, "materials/fogalomtar/glossary.json"),
    JSON.stringify({ terms: slugs.map((slug) => ({ slug })) }),
  );
}
...
test("visual coverage rejects missing overview, duplicate IDs, broken shared links and chart without source", () => {
  ...
  routes[0] = route("/", {
    ...
    visualQuestions: [
      {
        id: "q",
        question: "Mi történik?",
        type: "process",
        glossarySlugs: ["rug"],
        coverage: "shared",
        diagramId: "fig-shared",
        sharedHref: "../masik/#wrong",
        takeaway: "Lépések.",
      },
      {
        id: "q",
...
test("glossaryCoverage all follows the checked-out glossary registry without a frozen slug list", () => {
  const routes = canonicalRoutes();
  routes[0] = route("/", {
    visualQuestions: [
      {
        id: "q-all-glossary",
        question: "Hol találkozol a fogalmakkal?",
        type: "relationship",
        glossaryCoverage: "all",
        coverage: "page-local",
        diagramId: "fig-overview-root",
        takeaway: "Minden fogalom tanulási helyet kap.",
      },
    ],
  });
  const root = fixture(routes);
  writeGlossary(root, ["agent-ready-repo", "rug"]);
  assert.deepEqual(
    validateManifest({
      source: root,
      site: path.join(root, ".site"),
  ...
  writeGlossary(root, ["agent-ready-repo", "rug", "reinforced-human-learning"]);
  assert.deepEqual(
    validateManifest({
      source: root,
      site: path.join(root, ".site"),
...
test("glossaryCoverage all rejects a duplicated frozen glossarySlugs list", () => {
  const routes = canonicalRoutes();
  routes[0] = route("/", {
    visualQuestions: [
      {
  ...
  const root = fixture(routes);
  writeGlossary(root, ["agent-ready-repo"]);
  const failures = validateManifest({
    source: root,
    site: path.join(root, ".site"),
    phase: "foundation",
  ...
  assert.ok(
    failures.some((failure) => failure.includes("cannot freeze glossarySlugs")),
  );
});
...
test("foundation rejects omitted canonical routes and wrong frozen ownership", () => {
  ...
  assert.ok(
    failures.some((x) => x.includes("exactly 33 frozen canonical routes")),
  );
  assert.ok(failures.some((x) => x.includes("frozen owner mismatch")));
  assert.ok(
    failures.some((x) =>
      x.includes("missing frozen canonical route: /reference-app/"),
    ),
  );
});
...
test("final rejects an empty generated site even when every canonical source exists", () => {
  ...
  assert.ok(
    failures.some((x) => x.includes("final output missing: index.html")),
  );
...
test("compatibility metadata rejects missing target, duplicate old path, wrong sunset/canonical and unsafe absolute output", () => {
  ...
  assert.ok(
    failures.some((x) =>
      x.includes("canonical target is not a manifest route"),
    ),
  );
