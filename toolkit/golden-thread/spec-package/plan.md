...
- Exact check commands confirmed: `npm run typecheck`, `npm run lint`, `npm run test`,
  `npm run build`.
...
| 4 | API routes: `GET/POST /api/registrations`, `POST /api/registrations/[id]/cancel` — composition root wires the file adapter | AC-01, AC-02, AC-03 (HTTP contract) | `npm run typecheck` + manual `Invoke-RestMethod` probes | résztvevő |
| 5 | `/regisztracio` page: form, workshop select, active list with cancel, visible error surface (Hungarian UI copy) | AC-01, AC-02, AC-03 (observable UI) | `npm run lint && npm run build` + browser check | résztvevő |
