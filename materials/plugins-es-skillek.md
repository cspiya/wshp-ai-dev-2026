...
| **vercel** | Komplett skill-arzenál: deploy, env-kezelés, bootstrap, Next.js/shadcn/Turbopack guidance, `verification` (end-to-end ellenőrzés) — ~30 skill | A stackünk deploy-oldala; a Neon⇄Vercel preview-plumbing (Day 2) munkájához kulcs |
| **neon** | Neon MCP + guidance (branch-kezelés, serverless Postgres) | Az adatbázis-oldal: branch-per-preview a workflow centerpiece-e |
...
*Megjegyzés a `vercel` pluginhez: egyetlen plugin ~30 skillt hozott be — jó élő példa a lenti 1. korlátra
(a skill-menü kontextus-költsége). Hasznos, de látszik rajta, milyen gyorsan duzzad a lista.*
