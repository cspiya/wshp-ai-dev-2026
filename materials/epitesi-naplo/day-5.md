...
flowchart LR
    ...
    PACKAGES --> COMPOSED["🧩 Összeállt main<br/>hiba → javítás → új bizonyítás"]
```
...
<details>
<summary>🤖 <b>A6 · Az összeállt main felülírta a külön branch-ek bizonyosságát</b> [post-merge] [quality-gate] [publikálás]</summary>

A három nagy tananyagcsomag külön-külön review-n és saját kapukon ment át, de az egymás után merge-elt
változások új szerződésütközést hoztak létre. A fogalomtár új kattintható SVG-elemei még nem használták
a közben szigorított `node-shape` hover- és fókuszhorgot. A végső `main`-ellenőrzés ezt helyesen
megállította, ezért a GitHub Pages nem publikálta a hiányos változatot.

A javítás egyetlen release-hardening csomagban kezeli a teljes ok-okozati láncot: a tizenöt linkelt
alak szemantikus szerepet kapott, a no-CLI kapu már a flaggel indított Node-parancsokat is felismeri,
a negatív fixture-ök mindegyikének külön el kell buknia, és a publikus rétegből kikerültek a belső
feladatazonosítók. A tanulság: függő branchek esetén a branch-zöldség nem kiadási bizonyíték; az
összeállt `main` teljes kapuja és a tényleges publikáció együtt zárja le a munkát.
</details>
...
ív, valamint az eszköz–fogalomtár–starter csomag. A három csomag merge-e után az összeállított
`main`-ellenőrzés valóban talált egy kereszt-branch diagramhibát, és megakadályozta a publikációt.
A release-hardening csomag lezárását ezért már csak a teljes zöld kapu, a független review és a
tényleges GitHub Pages-verzió együtt bizonyíthatja.
